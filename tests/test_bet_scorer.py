...
from app.domain import OddsPhase, OddsSnapshot
...
def test_a_tier_after_draft_good_odds_has_high_score() -> None:
    score = score_odds_snapshot(
        make_snapshot(),
        {"total_kills_over": 8, "hype_penalty": -4},
    )
...
def test_streamer_over_kills_increases_total_kills_over_score() -> None:
    without_signal = score_odds_snapshot(make_snapshot(), {})
    with_signal = score_odds_snapshot(make_snapshot(), {"total_kills_over": 8})
